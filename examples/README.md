...
| 03 | [`03_tracking.php`](03_tracking.php) | Both tracking views (milestones and the raw event feed), the refined delivery ETA, the signer, and the two customer-facing tracking links | Yes (sandbox) |
...
| 09 | [`09_validation_and_errors.php`](09_validation_and_errors.php) | Every local business rule triggered in turn, quotation rules vs. booking rules, and the whole exception hierarchy with what each type carries | **No** |
| 10 | [`10_webhook_endpoint.php`](10_webhook_endpoint.php) | A Track & Trace webhook endpoint: parsing the payload, matching it to your own order, reacting to ETAs and proof of delivery, and the authentication caveat | **No** |
